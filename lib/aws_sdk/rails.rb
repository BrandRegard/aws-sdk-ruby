...
module AWS_SDK
  ...
  module Rails
    ...
    #   {AWS_SDK::SimpleEmailService#new} before delivering email.
    #
    # @return [nil]
    #
    def self.add_action_mailer_delivery_method name = :amazon_ses, options = {}
      ...
      if ::Rails.version.to_f >= 3
        ActiveSupport.on_load(:action_mailer) do
          self.add_delivery_method(name, AWS_SDK::SimpleEmailService, options)
        end
      else
        amb = ::ActionMailer::Base
        amb.send(:define_method, "perform_delivery_#{name}") do |mail|
          AWS_SDK::SimpleEmailService.new(options).send_raw_email(mail)
        end
      end
