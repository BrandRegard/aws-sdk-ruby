...
module AWS_SDK
  module Core
    ...
    #
    #     s3 = AWS_SDK::S3.new(:s3_server_side_encryption => :aes256)
    #
    ...
    class Configuration
      ...
      # AWS objects:
      #
      #   AWS_SDK::S3.new(:config => no_retries_config)
      #   AWS_SDK::SQS.new(:config => no_retries_config)
      #
      # @param options (see AWS.config)
      # @option options (see AWS.config)
      ...
      class << self
        ...
        def add_service name, ruby_name, default_endpoint
          ...
          create_block = lambda do |config,client_options|
            AWS_SDK.const_get(name)::Client.new(:config => config)
          end
