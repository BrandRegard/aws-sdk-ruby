...
Around("@orm", "@hash_model") do |scenario,block|
  ...
  class ExampleClass < AWS_SDK::Record::HashModel; end
...
Given /^I set the AWS_SDK::Record\.table_name_prefix to "([^"]*)"$/ do |prefix|
  AWS_SDK::Record.table_prefix = prefix
end
...
When /^I should be able to reset the table prefix$/ do
  AWS_SDK::Record.table_prefix = nil
end
