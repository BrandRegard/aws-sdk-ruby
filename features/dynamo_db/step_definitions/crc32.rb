...
def build_dynamo_db_crc32(num_failing_reqs = 1, crc_enabled = true)
  @crc_handler = AWS_SDK::Core::Http::Handler.new(@http_handler) do |req, resp, read_block|
  ...
  @dynamo_db = AWS_SDK::DynamoDB.new :use_ssl => false, :max_retries => 2,
    :dynamo_db_crc32 => crc_enabled, :http_handler => @crc_handler
end

Given /^I turn SSL off$/ do
  @dynamo_db = AWS_SDK::DynamoDB.new(:config => @dynamo_db.config.with(:use_ssl => false))
end
...
Then /^the request should fail with a CRC checking error$/ do
  @error.should be_a(AWS_SDK::DynamoDB::Errors::CRC32CheckFailed)
end
